javascript:(function(){function l(e){["pointerdown","mousedown","mouseup","click"].forEach(t=>{e.dispatchEvent(new MouseEvent(t,{bubbles:!0,cancelable:!0,view:window,button:0}))})}function m(){const e=["第1回公演","第2回公演","第3回公演","第4回公演","第5回公演"],t=[["いない","３才以下の子どもはいない"],["1人","チケットをお持ちの方１人につき、３才以下の子どもが１人"],["2人","チケットをお持ちの方１人につき、３才以下の子どもが２人"]],n=document.createElement("div");n.style.position="fixed",n.style.top="10px",n.style.right="10px",n.style.backgroundColor="#fff",n.style.border="1px solid #ccc",n.style.padding="10px",n.style.zIndex="9999",n.style.borderRadius="8px",n.style.boxShadow="0 2px 6px rgba(0,0,0,0.3)",n.style.fontSize="14px",n.style.fontFamily="sans-serif",n.style.display="flex",n.style.flexDirecti
// ==UserScript==
...
(function () {
  ...
  async function waitForOverlay() {
    try {
      const overlay = await waitForElement(".v-overlay.v-overlay--active", 15000);
      console.log("✅ ポップアップ表示を検知しました");
      return overlay;
    } catch (err) {
      console.error("❌ ポップアップ検出に失敗しました");
      throw err;
    }
  }
  ...
  async function setTicketQuantity() {
    ...
    const addButton = document.querySelector('.icon__add');
    const removeButton = document.querySelector('.icon__remove');
    let current = parseInt(valueSpan.textContent);
    while (current < ticketCount) {
      simulateFullUserClick(addButton);
      await new Promise(r => setTimeout(r, 50));
      current++;
    }
    while (current > ticketCount) {
      simulateFullUserClick(removeButton);
      await new Promise(r => setTimeout(r, 50));
      current--;
    }
  ...
  async function waitAndClickConfirm() {
    for (let i = 0; i < 20; i++) {
      ...
      if (btn && !btn.disabled && btn.style.opacity === "1") {
        ...
        console.log("🪑 座席を確認するボタンをクリックしました");
        return;
      }
      await new Promise(r => setTimeout(r, 500));
    }
    console.error("❌ ボタンが有効にならずタイムアウトしました");
  }

  async function fullAutomationFlow() {
    if (!clickPerformanceCard()) return;
    await waitForOverlay();
    await new Promise(r => setTimeout(r, 100)); // 安定のための小休止
